...
import { useRequestSimulationUpload, useConfirmSimulationUpload } from '@/lib/queries/simulation-attachments';
...
export function SimulationAttachmentUpload({ simulationId, onUploadComplete }: SimulationAttachmentUploadProps) {
  const [file, setFile] = useState<File | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [uploading, setUploading] = useState(false);

  const requestUpload = useRequestSimulationUpload(simulationId);
  const confirmUpload = useConfirmSimulationUpload(simulationId);
