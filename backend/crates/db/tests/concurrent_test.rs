...
async fn setup_concurrent_test_data(db: &DatabaseConnection) -> Result<ConcurrentTestData, sea_orm::DbErr> {
    ...
    chart_of_accounts::ActiveModel {
        ...
        organization_id: Set(org_id),
        code: Set(format!("1000-C-{}", Uuid::new_v4().to_string()[..6].to_string())),
    ...
    chart_of_accounts::ActiveModel {
        ...
        organization_id: Set(org_id),
        code: Set(format!("5000-C-{}", Uuid::new_v4().to_string()[..6].to_string())),
...
async fn cleanup_concurrent_test_data(db: &DatabaseConnection, data: &ConcurrentTestData) -> Result<(), sea_orm::DbErr> {
...
async fn get_account_balance(db: &DatabaseConnection, account_id: Uuid) -> Result<Decimal, sea_orm::DbErr> {
    ...
    Ok(entry.map(|e| e.account_current_balance).unwrap_or(Decimal::ZERO))
}

/// Gets the highest account version for an account.
async fn get_account_version(db: &DatabaseConnection, account_id: Uuid) -> Result<i64, sea_orm::DbErr> {
    let entry = ledger_entries::Entity::find()
        .filter(ledger_entries::Column::AccountId.eq(account_id))
        .order_by_desc(ledger_entries::Column::AccountVersion)
        .one(db)
...
async fn test_concurrent_100_transactions_correct_balance() {
    ...
    println!("Completed {} of {} transactions", success_count, NUM_TRANSACTIONS);
...
async fn test_stress_1000_concurrent_transactions() {
    ...
    println!("Starting stress test with {} transactions in batches of {}", NUM_TRANSACTIONS, BATCH_SIZE);
    ...
    for batch in 0..(NUM_TRANSACTIONS / BATCH_SIZE) {
        ...
        if (batch + 1) % 5 == 0 {
            println!("  Completed batch {}/{}", batch + 1, NUM_TRANSACTIONS / BATCH_SIZE);
        }
    }
    
    println!("Stress test completed: {} success, {} failures", total_success, total_failure);
...
async fn test_concurrent_running_balance_consistency() {
    ...
    let results: Vec<Result<Result<Uuid, sea_orm::DbErr>, tokio::task::JoinError>> = join_all(handles).await;
...
async fn test_concurrent_version_monotonicity() {
    ...
    for (i, &version) in versions.iter().enumerate() {
        ...
        assert_eq!(
            ...
            i, expected, version, &versions[..std::cmp::min(10, versions.len())]
        );
    }
    ...
    assert_eq!(
        sorted_versions.len(), versions.len(),
        "Duplicate versions detected! Original: {}, Unique: {}",
        versions.len(), sorted_versions.len()
    );
